...
class ArtificialIntelligenceAlgorithm {
  ...
  static List<int> randomMove(List<List<Tile>> tiles) {
    ...
    if ((newY < height && newY >= 0) && (newX < width && newX >= 0)) {
      ...
      coordinates = randomMove(tiles);
    }
    return coordinates;
  }

  static List<int> easyMove(List<List<Tile>> tiles) {
    ...
    if (coordinates[0] == -1) { // If no valid attacking move was found
  ...
  static List<int> getBiggestAIBall(List<List<Tile>> tiles) {
    List<List<int>> aiBalls = _getAIBalls(tiles);
    if (aiBalls.isEmpty) {
  ...
  static List<int> hardMove(List<List<Tile>> tiles, bool chaser) {
    ...
    for (List<int> pos in aiBallPositions) {
      ...
      List<List<int>> splitDeltas = [[0, 2], [0, -2], [2, 0], [-2, 0]];
      for (List<int> deltaPair in splitDeltas) {
          ...
          if ((targetY < height && targetY >= 0) && (targetX < width && targetX >= 0)) {
            ...
            if (targetBall is BallGreen && targetBall.size <= (aiBall.size ~/ 3) && aiBall.size >= 10) { // Min size for split? GameManager had >=10
  ...
  static List<int> _chaserMove(List<List<Tile>> tiles) {
    List<int> aiBiggestBallPos = getBiggestAIBall(tiles);
    if (aiBiggestBallPos[0] == -1) return randomMove(tiles); // No AI balls

    Ball? aiBall = tiles[aiBiggestBallPos[0]][aiBiggestBallPos[1]].ball;
    if (aiBall == null) return randomMove(tiles);


    List<List<int>> playerBallPositions = _getPlayerBalls(tiles);
    if (playerBallPositions.isEmpty) {
      return randomMove(tiles); // No player balls to chase
    }
    ...
    for (List<int> playerPos in playerBallPositions) {
      ...
      if (aiBall.size > playerBall.size) { // Only chase if AI ball is bigger
    ...
    if (targetY < currentY) {
      ...
      finalY = currentY + 1;
    } else if (targetX < currentX) { // Same row, try to move horizontally
      finalX = currentX - 1;
